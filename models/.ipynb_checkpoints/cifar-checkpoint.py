...
import numpy as np
from models.noisy_mixup import do_noisy_mixup
...
class ResNet(nn.Module):
    def __init__(self, opt):
        ...
        if self.nettype == "cifar":
            ...
            self.blocks = [self.group1, self.group2, self.group3]
            ...
            self.blocks = [self.group1, self.group2, self.group3, self.group4]
    ...
    def forward_nfm(self, x, targets=None, mixup_alpha=0.0, manifold_mixup=False, 
                add_noise_level=0.0, mult_noise_level=0.0): 
        
        k = 0 if mixup_alpha>0.0 else -1
        
        if self.nettype == "imagenet":        
            if mixup_alpha > 0.0 and manifold_mixup == True: k = np.random.choice(range(len(self.blocks)-1), 1)[0]
        elif self.nettype == "cifar":        
            if mixup_alpha > 0.0 and manifold_mixup == True: k = np.random.choice(range(len(self.blocks)-1), 1)[0]
