...
class ER_NFM(ER):
    def __init__(
        ...
        self.manifold_mixup=kwargs['manifold_mixup']
        self.add_noise_level=kwargs['add_noise_level']
        self.mult_noise_level=kwargs['mult_noise_level']                    
        """ """ """ """
    ...
    def model_forward(self, x, y):
        ...
        if do_cutmix:
            ...
            if self.use_amp:
                with torch.cuda.amp.autocast():
                    logit, labels_a, labels_b, lam = self.model.forward_nfm(x, targets=y, mixup_alpha=self.alpha,
                                                                      manifold_mixup=self.manifold_mixup,
                                                                      add_noise_level=self.add_noise_level,
                                                                      mult_noise_level=self.mult_noise_level)                    
