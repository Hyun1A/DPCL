...
class BiasCorrection(ER):
    ...
    def online_evaluate(self, test_list, sample_num, batch_size, n_worker, end_task=False):
        ...
        eval_dict = self.get_avg_res(total_num_data, total_loss, total_correct, correct_l, num_data_l)        
        print('save result for task'+str(self.cur_iter+1))
        self.save_results(eval_dict, end_task)
        self.save_results(eval_dict, end_task, islatest=True)
        
        self.writer.add_scalar(f"test/loss", eval_dict["avg_loss"], sample_num)
        self.writer.add_scalar(f"test/acc", eval_dict["avg_acc"], sample_num)
        logger.info(
    ...
    def online_bias_correction(self, n_iter=256, batch_size=100, n_worker=4):
        ...
        if self.val_full and self.cur_iter > 0 and len(self.bias_labels[self.cur_iter]) > 0:
            val_df = pd.DataFrame(self.valid_list)
            val_dataset = ImageDataset(self.root, val_df, dataset=self.dataset, transform=self.test_transform,
