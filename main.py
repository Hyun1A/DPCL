...
def main():
    ...
    res_path =  f"{args.root}/{args.exp_name}/results/{args.dataset}/{args.mode}/memory_size_{args.memory_size}/seed_{args.rnd_seed}/{args.tm}/"
    res_path_latest =  f"{args.root}/{args.exp_name}/results/{args.dataset}/{args.mode}/memory_size_{args.memory_size}/seed_{args.rnd_seed}/latest/"
    
    tensorboard_path = f"{args.root}/{args.exp_name}/tensorboard/{args.dataset}/{args.mode}/memory_size_{args.memory_size}/{args.rnd_seed}/{args.tm}/"
    ...
    method = select_method(
        args, criterion, device, train_transform, test_transform, n_classes, writer
    )    
   ...
   #     test_datalist = test_datalist[:1000] + test_datalist[5000:5200] + test_datalist[6000:6200] + test_datalist[7000:7200] + test_datalist[8000:8200] + test_datalist[9000:9200]
    ...
    for cur_iter in range(args.n_tasks):
        ...
        if args.debug:
            cur_train_datalist = cur_train_datalist[:320]
            random.shuffle(test_datalist_debug)
            test_datalist = test_datalist_debug[:320]# + test_datalist_debug[-150:]
        ...
        eval_dict = method.online_evaluate(test_datalist, samples_cnt, 512, args.n_worker, end_task=True)
        
        
        if args.mode != 'gdumb':
            task_records = append_task_records(task_records, eval_dict, cur_iter, logger, writer)
    
    if 'gdumb' in args.mode:
        print(args.mode)
